...
@section('content')
    <div class="d-flex align-items-center mb-4">
        <a href="{{ route('live-lectures.index') }}" class="btn btn-light rounded-circle shadow-sm me-3"
            ...
            <div class="card border-0 shadow-sm rounded-4 p-4">
                <form action="{{ route('live-lectures.store') }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            <label class="form-label fw-semibold text-secondary small text-uppercase">Subject <span
                                    class="text-danger">*</span></label>
                            <select name="subject"
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->name }}" data-batch="{{ $subject->batch_id }}" {{ old('subject') == $subject->name ? 'selected' : '' }}>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-semibold text-secondary small text-uppercase">Lecture Title <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="description"
                            ...
                            rows="3" placeholder="What will be covered in this lecture?">{{ old('description') }}</textarea>
                        @error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="alert alert-info rounded-4 border-0 bg-primary bg-opacity-10">
                        <i class="fas fa-info-circle me-2 text-primary"></i>
                        <strong>How it works:</strong> Scheduling creates the lecture room. When you're ready to teach,
                        click <strong>"Go Live"</strong> on the index page to open the interactive video classroom — your
                        students will see a join button instantly!
                    </div>

                    <div class="text-end mt-4">
                        <a href="{{ route('live-lectures.index') }}"
                            class="btn btn-light btn-lg rounded-pill px-4 me-2 fw-semibold">Cancel</a>
                        <button type="submit" class="btn btn-primary-glow btn-modern btn-lg px-5">Schedule Lecture <i
                                class="fas fa-calendar-check ms-2"></i></button>
                    </div>
                </form>
            </div>
        </div>
