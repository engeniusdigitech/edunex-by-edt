...
@section('content')
    <div class="mb-4">
        <a href="{{ route('payments.index') }}" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i> Back to
            Ledger</a>
    </div>
    ...
    <div class="card border-0 shadow-sm col-md-8">
        ...
        <div class="card-body">
            <form action="{{ route('payments.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="fee_structure_id" class="form-select" required>
                        ...
                        @foreach($feeStructures as $fee)
                            <option value="{{ $fee->id }}" {{ request('fee_structure_id') == $fee->id ? 'selected' : '' }}>
                                {{ $fee->name }} (Total: ₹{{ $fee->total_amount }})
                            </option>
                        @endforeach
                    </select>
                ...
                <button type="submit" class="btn btn-primary btn-lg w-100"><i class="fas fa-check"></i> Complete
                    Transaction</button>
            </form>
        </div>
    </div>
