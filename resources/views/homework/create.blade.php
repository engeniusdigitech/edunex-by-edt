        ...
        <div class="card border-0 shadow-sm rounded-4 p-4">
            <form action="{{ route('homework.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" class="form-control form-control-lg rounded-3 @error('title') is-invalid @enderror" value="{{ old('title') }}" placeholder="e.g. Chapter 4 Algebra Exercises" required>
                    @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-4">
                    <label class="form-label fw-semibold text-secondary small text-uppercase">Description / Instructions</label>
                    <textarea name="description" class="form-control form-control-lg rounded-3 @error('description') is-invalid @enderror" rows="4" placeholder="Provide details, page numbers, or instructions here...">{{ old('description') }}</textarea>
                ...
                <div class="mb-5">
                    ...
                    <input type="date" name="due_date" class="form-control form-control-lg rounded-3 @error('due_date') is-invalid @enderror" value="{{ old('due_date', now()->addDays(2)->format('Y-m-d')) }}" required>
                    @error('due_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="text-end">
                    <a href="{{ route('homework.index') }}" class="btn btn-light btn-lg rounded-pill px-4 me-2 fw-semibold">Cancel</a>
                    <button type="submit" class="btn btn-primary-glow btn-modern btn-lg px-5">Publish Homework <i class="fas fa-paper-plane ms-2"></i></button>
                </div>
            </form>
        </div>
    </div>
