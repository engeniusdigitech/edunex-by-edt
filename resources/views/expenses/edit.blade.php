...
@section('title', 'Modify Expense Voucher')
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
            <div class="card-header bg-white border-bottom-0 pt-4 px-4 pb-0">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="fw-bold text-dark mb-0"><i class="fas fa-edit text-primary me-2"></i>Modify Expense Voucher</h5>
                        <p class="text-muted small mb-0">Update operating outflow parameters and automatically re-post ledger entries</p>
                    </div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('expenses.update', $expense->id) }}" method="POST" id="expense-form">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <select name="accounting_ledger_id" class="form-select @error('accounting_ledger_id') is-invalid @enderror" required>
                                ...
                                @foreach($expenseLedgers as $ledger)
                                    <option value="{{ $ledger->id }}" {{ old('accounting_ledger_id', $expense->accounting_ledger_id) == $ledger->id ? 'selected' : '' }}>
                                        {{ $ledger->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <select name="payment_method" class="form-select @error('payment_method') is-invalid @enderror" required>
                                <option value="Cash" {{ old('payment_method', $expense->payment_method) === 'Cash' ? 'selected' : '' }}>Cash-in-Hand</option>
                                <option value="Bank" {{ old('payment_method', $expense->payment_method) === 'Bank' ? 'selected' : '' }}>Bank Account (Transfer / Check)</option>
                                <option value="Card" {{ old('payment_method', $expense->payment_method) === 'Card' ? 'selected' : '' }}>Corporate Card</option>
                            </select>
                    ...
                    <div class="row g-3 mb-3 bg-light p-3 rounded-4 border border-light-subtle">
                        ...
                        <div class="col-12 col-sm-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0" name="net_amount" id="net_amount" class="form-control border-start-0 @error('net_amount') is-invalid @enderror" value="{{ old('net_amount', $expense->net_amount) }}" placeholder="0.00" required>
                            </div>
                            @error('net_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12 col-sm-4">
                            ...
                            <select name="gst_rate" id="gst_rate" class="form-select @error('gst_rate') is-invalid @enderror" required>
                                <option value="0" {{ old('gst_rate', round($expense->gst_rate)) == 0 ? 'selected' : '' }}>Exempt / 0%</option>
                                <option value="5" {{ old('gst_rate', round($expense->gst_rate)) == 5 ? 'selected' : '' }}>5% GST</option>
                                <option value="12" {{ old('gst_rate', round($expense->gst_rate)) == 12 ? 'selected' : '' }}>12% GST</option>
                                <option value="18" {{ old('gst_rate', round($expense->gst_rate)) == 18 ? 'selected' : '' }}>18% GST (Standard Services)</option>
                                <option value="28" {{ old('gst_rate', round($expense->gst_rate)) == 28 ? 'selected' : '' }}>28% GST</option>
                            </select>
                            @error('gst_rate')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12 col-sm-4">
                            ...
                            <select name="gst_type" id="gst_type" class="form-select @error('gst_type') is-invalid @enderror" required>
                                <option value="none" {{ old('gst_type', $expense->gst_type) === 'none' ? 'selected' : '' }}>None (No GST)</option>
                                <option value="cgst_sgst" {{ old('gst_type', $expense->gst_type) === 'cgst_sgst' ? 'selected' : '' }}>CGST + SGST (Intrastate)</option>
                                <option value="igst" {{ old('gst_type', $expense->gst_type) === 'igst' ? 'selected' : '' }}>IGST (Interstate)</option>
                            </select>
                            @error('gst_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="card border border-primary-subtle bg-primary-subtle p-3 mb-3 rounded-4" id="calc-preview-card">
                        <div class="d-flex justify-content-between text-xs text-primary-emphasis mb-1">
                            <span>Base Net Outflow:</span>
                            <span class="fw-semibold" id="preview-net">{{ currencySymbol() }}{{ number_format($expense->net_amount, 2) }}</span>
                        </div>
                        <div class="d-flex justify-content-between text-xs text-primary-emphasis mb-2">
                            <span>Input Tax portion (GST):</span>
                            <span class="fw-semibold" id="preview-gst">{{ currencySymbol() }}{{ number_format($expense->gst_amount, 2) }}</span>
                        </div>
                        <div class="d-flex justify-content-between text-sm text-primary-emphasis border-top pt-2">
                            ...
                            <span class="fw-bold fs-6" id="preview-total">{{ currencySymbol() }}{{ number_format($expense->total_amount, 2) }}</span>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label text-muted small fw-semibold">Invoice / Reference Number</label>
                        <input type="text" name="reference_no" class="form-control @error('reference_no') is-invalid @enderror" value="{{ old('reference_no', $expense->reference_no) }}" placeholder="e.g. INV-10029">
                        @error('reference_no')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" rows="3" class="form-control @error('description') is-invalid @enderror" placeholder="Provide transaction narration splits details...">{{ old('description', $expense->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary w-100 rounded-pill py-2.5 shadow-sm">
                        <i class="fas fa-check-circle me-1"></i>Update Expense Voucher
                    </button>
                </form>
            </div>
        </div>
