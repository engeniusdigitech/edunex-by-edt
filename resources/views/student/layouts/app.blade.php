...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .notif-icon-btn:hover, .nav-icon-btn:hover { background: var(--indigo); color: #fff; border-color: var(--indigo); }
        .notif-dot {
...
<header class="top-navbar">
    ...
    <div class="nav-right">
        ...
        @endif
        <a href="{{ route('student.fees.index') }}" class="nav-icon-btn {{ request()->routeIs('student.fees.*') ? 'bg-indigo text-white border-indigo' : '' }}" title="My Fees" style="{{ request()->routeIs('student.fees.*') ? 'background:var(--indigo);color:#fff;border-color:var(--indigo);' : '' }}">
            <i class="fas fa-wallet"></i>
        </a>
        <a href="{{ route('student.lectures.index') }}" class="nav-icon-btn {{ request()->routeIs('student.lectures.*') ? 'bg-indigo text-white border-indigo' : '' }}" title="Live Lectures" style="{{ request()->routeIs('student.lectures.*') ? 'background:var(--indigo);color:#fff;border-color:var(--indigo);' : '' }}">
        ...
        <form method="POST" action="{{ route('student.logout') }}" style="margin:0;">
            @csrf
            <button class="logout-btn" type="submit"><i class="fas fa-sign-out-alt"></i> <span class="d-none d-sm-inline">Logout</span></button>
        </form>
    </div>
</header>
