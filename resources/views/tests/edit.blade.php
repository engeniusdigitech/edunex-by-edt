...
@section('content')
    <div class="d-flex align-items-center mb-4">
        <a href="{{ route('tests.index') }}" class="btn btn-light rounded-circle shadow-sm me-3"
            style="width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
            <i class="fas fa-arrow-left text-secondary"></i>
        </a>
        <h4 class="mb-0 fw-medium">Edit Test</h4>
    </div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 p-4">
                <form action="{{ route('tests.update', $test) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-semibold text-secondary small text-uppercase">Test Title <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-secondary small text-uppercase">Test Date <span
                                    class="text-danger">*</span></label>
                            <input type="date" name="test_date"
                                class="form-control form-control-lg rounded-3 @error('test_date') is-invalid @enderror"
                                value="{{ old('test_date', $test->test_date->format('Y-m-d')) }}" required>
                            @error('test_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-secondary small text-uppercase">Total Marks <span
                                    class="text-danger">*</span></label>
                            <input type="number" name="total_marks"
                                class="form-control form-control-lg rounded-3 @error('total_marks') is-invalid @enderror"
                    ...
                    @if($test->attachments->count() > 0)
                        <div class="mb-4">
                            <label class="form-label fw-semibold text-secondary small text-uppercase">Current
                                Attachments</label>
                                    ...
                                    <div
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank"
                                                class="text-decoration-none small fw-semibold text-dark text-truncate"
                                                style="max-width: 240px;"
                                                title="{{ $attachment->original_name }}">{{ $attachment->original_name }}</a>
                                            <span class="text-muted ms-1"
                                                style="font-size: 0.75rem;">({{ number_format($attachment->file_size / 1024, 1) }}
                                                KB)</span>
                                        </div>
                                        <div>
                                            <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank"
                                                class="btn btn-sm btn-light rounded-circle"><i
                                                    class="fas fa-download fa-xs"></i></a>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="mb-5">
                        <label class="form-label fw-semibold text-secondary small text-uppercase">Syllabus /
                            Description</label>
                    ...
                    <div class="text-end">
                        ...
                        <button type="submit" class="btn btn-primary-glow btn-modern btn-lg px-5">Save Changes <i
